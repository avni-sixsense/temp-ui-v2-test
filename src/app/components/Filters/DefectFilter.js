...
const UseCaseFilter = ({ filterKey, data, isLoading, groundTruth }) => {
  const classes = useStyles();
  const dispatch = useDispatch();

  const filter = useSelector(({ filters }) =>
    groundTruth ? filters.groundTruth : filters.defect
  );
  ...
  useEffect(() => {
    ...
    if (params[filterKey] && !filter?.applied.length) {
      dispatch({
        type: 'APPLY_FILTER',
        field: groundTruth ? 'groundTruth' : 'defect',
  ...
  }, [location.search, filter, filterKey, groundTruth]);
  ...
  return (
    <Box mr={4} mb={1} className={classes.item}>
      <Box
        id={groundTruth ? 'filter_ground_truth' : 'filter_defect'}
        onClick={handleClick}
          ...
          <Autocomplete
            ...
            renderOption={(option, { selected }) => (
              <Box display='flex' alignItems='center'>
                <CustomizedCheckbox checked={selected} paddingZero />
