...
const ModelFilter = ({ filterKey, data, isLoading }) => {
  const classes = useStyles();
  const dispatch = useDispatch();

  const filter = useSelector(({ filters }) => filters.wafer);
  ...
  return (
    <Box mr={4} mb={1} className={classes.item}>
      <Box
        ...
        alignItems='center'
      >
        <Typography variant='h4'>Wafer map</Typography>
        <Box ml={1.5}>
      ...
      {open && (
        ...
        >
          <Autocomplete
            ...
            getOptionLabel={option => option.organization_wafer_id}
