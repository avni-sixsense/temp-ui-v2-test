...
const FailedMode = () => {
  ...
  const { data = {}, isLoading: isTasksLoading } = useQuery(
    ['failedTaskQueue', `?status=FAILURE&cursor=`],
  ...
  useEffect(() => {
    if (data.results) {
      const { results = [] } = data;
      dispatch(addTasks({ data: results || [], isNew: true }));
      dispatch(addNextPointer(data?.next));
    }
  }, [data]);

  useEffect(() => {
    const filteredTask = tasks.filter(task => task.status === 'FAILURE');
    dispatch(
      setPendingTasksIds(
        tasks
    ...
    if (!!nextPointer) {
      ...
      return axios
        ...
        .then(({ data: res }) => {
          ...
          dispatch(nextPointer(updateNextDataURL(res?.next)));
