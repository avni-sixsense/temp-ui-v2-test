...
const useStyles = makeStyles(theme => ({
  ...
  table: {
    ...
    '& [class*="MuiTableCell-sizeSmall-"]:last-child': {
      padding: '6px 6px 6px 0px',
      '& p': {
...
const mapTableToState = createStructuredSelector({
...
export default function Table({ columns, data, isLoading, selectable }) {
  ...
  const { selectedDefects } = useSelector(mapTableToState);
  ...
  useMemo(() => {
    ...
    if (
      ...
      dispatch(
        setSelectedDefects(
          selectedFlatRows.map(({ original: { id, name } }) => ({ id, name }))
        )
      );
    }
  }, [selectedFlatRows]);
                ...
                {headerGroup.headers.map(column =>
                  column.id === 'selection' ? (
                    ...
                    >
                      <Typography>
                        {column.render('Header')}
                        <span style={{ marginLeft: 3, color: '#6F8EBD' }}>
                          {column.isSorted ? (
                            column.isSortedDesc ? (
